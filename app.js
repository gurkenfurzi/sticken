...
const defaults = {
  orders:[
    {id:"o1",name:"Frigo Trans",phone:"01701234567",accepted:todayISO(),due:todayISO(),price:120,location:"Regal A",text:"Dubbeglas mit Druck",status:"progress",photos:[],informed:false,ready:false,reminder:false,reminderAt:null,reminded:false,paid:false},
    {id:"o2",name:"Klara Meier",phone:"01721234567",accepted:todayISO(),due:new Date(Date.now()+86400000).toISOString().slice(0,10),price:42,location:"Regal B · Fach 2",text:"1× Schorli Kuscheltier mit Name.",status:"open",photos:[],informed:false,ready:false,reminder:false,reminderAt:null,reminded:false,paid:false}
  ],
  supplies:[
...
function newOrder(){
  ...
  layout(`
    ...
    <form id="orderForm" class="form">
      <div class="field"><label>Name / Firma</label><input name="name" required></div>
      <div class="field"><label>Telefonnummer</label><input name="phone" inputmode="tel"></div>
      <div class="grid2">
...
function calendar(){
  ...
  function drawSchedule(){
    ...
    $$('[data-event-open]').forEach(card => card.addEventListener('click', () => { if(card.dataset.eventOpen){ route = {page:'detail', id:card.dataset.eventOpen}; detail(card.dataset.eventOpen); } }));
  }
  function draw(){
...
function supplies(){
  ...
  const render = () => {
    ...
    $('#supplyList').innerHTML = list.length ? list.map(s => `<div class="card supply-card">${s.photo ? `<img class="thumb" src="${s.photo}">` : `<div class="thumb placeholder">${ICONS.bag}</div>`}<div><strong>${escapeHTML(s.name)}</strong><br><small>Menge: ${escapeHTML(s.amount)}</small></div><button class="secondary-btn pressable" data-supply-toggle="${s.id}">${s.ordered ? 'Offen' : 'Bestellt'}</button></div>`).join('') : `<div class="card empty">Keine Einträge.</div>`;
    $$('[data-supply-toggle]').forEach(btn => btn.addEventListener('click', () => { const s = state.supplies.find(x => x.id === btn.dataset.supplyToggle); s.ordered = !s.ordered; saveState(); toast(s.ordered ? 'Als bestellt markiert' : 'Wieder offen'); render(); }));
  };
...
function more(){
  ...
  $('[data-action="reset"]').addEventListener('click', () => { if(confirm('Beispieldaten wirklich zurücksetzen?')){ state = structuredClone(defaults); saveState(); setTheme(state.settings.theme); scheduleReminderChecks(); home(); } });
}
...
function bindModal(){
  ...
  if(modal.type === 'event'){
    $('#eventForm').addEventListener('submit', e => { e.preventDefault(); const f = new FormData(e.currentTarget); state.events.push({id:uid(), date:f.get('date'), time:f.get('time'), title:String(f.get('title')||'').trim(), note:String(f.get('note')||'').trim()}); saveState(); modal = null; selectedDate = f.get('date'); toast('Termin gespeichert'); calendar(); });
  }
  if(modal.type === 'supply'){
    let photo = '';
  ...
  if(modal.type === 'reminder'){
    const o = state.orders.find(x => x.id === modal.id);
    $('#reminderForm').addEventListener('submit', async e => { e.preventDefault(); const f = new FormData(e.currentTarget); o.reminder = true; o.reminderAt = `${f.get('date')}T${f.get('time')}:00`; o.reminded = false; if('Notification' in window){ try{ if(Notification.permission !== 'granted') await Notification.requestPermission(); }catch(err){} } saveState(); scheduleReminderChecks(); modal = null; toast('Erinnerung gespeichert'); detail(o.id); });
...
function bindGlobal(){
  ...
  $$('[data-back]').forEach(btn => btn.addEventListener('click', () => { if(route.page === 'detail') go('orders'); else if(route.page === 'prices') go('more'); else go('home'); }));
